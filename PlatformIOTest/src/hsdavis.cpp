...
int readDavis()
{
    ...
    { // If anything comes in Serial1
        ...
        {                                 // If anything comes in Serial1
            ...
            davisData.wInsideTemp = toInt16(buffer, 9);   /* int16_t 9 Inside Temperature as (DegF / 10)          */
            davisData.yInsideHum = buffer[11];            /* uint8_t 11 Inside Humidity as percentage             */
            davisData.wOutsideTemp = toInt16(buffer, 12); /* int16_t 12 Outside Temperature as (DegF / 10)        */
            ...
            davisData.wWindDir = toUInt16(buffer, 16);    /* uint16_t 16 Wind Direction in degress                */
            davisData.yOutsideHum = buffer[33];           /* uint8_t 33 Outside Humidity                          */
            davisData.wRainRate = toUInt16(buffer, 41);   /* uint16_t 41 Rain Rate                                */
            davisData.yUVLevel = buffer[43];              /* uint8_t 43 UV Level                                  */
            davisData.wSolarRad = toUInt16(buffer, 44);   /* uint16_t 44 Solar Radiation                          */
            davisData.wStormRain = toUInt16(buffer, 46);  /* uint16_t 46 Total Storm Rain                         */
            davisData.wStormStart = toUInt16(buffer, 48); /* uint16_t 48 Start date of current storm              */
            davisData.wRainDay = toUInt16(buffer, 50);    /* uint16_t 50 Rain Today                               */
